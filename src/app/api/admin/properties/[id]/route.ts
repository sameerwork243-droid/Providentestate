...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const item = rows("SELECT * FROM properties WHERE id = ?", Number(id))[0];
  ...
  const media = rows("SELECT * FROM property_media WHERE property_id = ? ORDER BY sort_order, id", Number(id));
